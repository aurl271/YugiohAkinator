...
function c94801854.spcfilter(c,tp)
	return c:IsRace(RACE_ZOMBIE)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c94801854.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c94801854.spcfilter,1,nil,tp) end
	local sg=Duel.SelectReleaseGroup(tp,c94801854.spcfilter,1,1,nil,tp)
