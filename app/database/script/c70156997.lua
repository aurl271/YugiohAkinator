...
function c70156997.cfilter(c,tp)
	return c:IsAttribute(ATTRIBUTE_EARTH)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c70156997.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c70156997.cfilter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c70156997.cfilter,1,1,nil,tp)
