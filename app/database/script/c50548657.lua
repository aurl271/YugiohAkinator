...
function c50548657.costfilter(c,e,tp,g,max)
	local lv=c:GetLevel()
	local ft=math.min((Duel.GetMZoneCount(tp,c)),max)
	return c:IsLevelBelow(4) and c:IsRace(RACE_CYBERSE) and ft>0 and (c:IsControler(tp) or c:IsFaceup())
...
function c50548657.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.CheckReleaseGroup(tp,c50548657.costfilter,1,nil,e,tp,g,max) end
	local sg=Duel.SelectReleaseGroup(tp,c50548657.costfilter,1,1,nil,e,tp,g,max)
