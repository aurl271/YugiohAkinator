...
function c286392.cfilter(c,tp)
	return c:IsLevelBelow(2)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c286392.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c286392.cfilter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c286392.cfilter,1,1,nil,tp)
