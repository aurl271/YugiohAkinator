...
function c2377034.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x2c)
end
function c39761138.rmcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsRelateToBattle() and c:GetBattleTarget():IsType(TYPE_MONSTER)
		and Duel.IsExistingMatchingCard(c2377034.filter,tp,LOCATION_GRAVE,0,3,nil)
end
...
function c39761138.rmop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(c2377034.filter,tp,LOCATION_GRAVE,0,3,nil) then return end
