...
function c36690018.filter(c)
	return c:IsFaceup() and c:IsSetCard(0x5) and c:GetFlagEffect(FLAG_ID_ARCANA_COIN)~=0
end
...
function c36690018.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) and tc:IsFaceup() and (tc:GetFlagEffect(FLAG_ID_REVERSAL_OF_FATE)~=0 or tc:GetFlagEffect(FLAG_ID_ARCANA_COIN)~=0) then
