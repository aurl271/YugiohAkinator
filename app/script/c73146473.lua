...
function c73146473.cfilter(c,tp)
...
function c73146473.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c73146473.cfilter(chkc,tp) end
	if chk==0 then return Duel.IsExistingTarget(c73146473.cfilter,tp,LOCATION_MZONE,0,1,nil,tp)
		and Duel.IsExistingMatchingCard(c73146473.filter,tp,LOCATION_HAND,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectTarget(tp,c73146473.cfilter,tp,LOCATION_MZONE,0,1,1,nil,tp)
