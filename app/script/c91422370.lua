...
function c91422370.costfilter(c,tp)
	return c:IsSetCard(0x58) and c:IsAttackAbove(1500)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c91422370.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c91422370.costfilter,1,nil,tp) end
	local sg=Duel.SelectReleaseGroup(tp,c91422370.costfilter,1,1,nil,tp)
