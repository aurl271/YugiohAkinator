...
function c23408872.cfilter(c,tp)
	return c:IsRace(RACE_AQUA)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c23408872.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c23408872.cfilter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c23408872.cfilter,1,1,nil,tp)
