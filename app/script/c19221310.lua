...
function c19221310.cfilter(c,tp)
	return c:IsAttribute(ATTRIBUTE_LIGHT) and (c:IsControler(tp) or c:IsFaceup())
		and Duel.IsExistingMatchingCard(c19221310.filter,tp,LOCATION_MZONE+LOCATION_HAND+LOCATION_DECK,0,1,c,tp,c)
end
function c19221310.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c19221310.cfilter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c19221310.cfilter,1,1,nil,tp)
	Duel.Release(g,REASON_COST)
end
function c19221310.filter(c,tp,tc)
...
function c19221310.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,c19221310.filter,tp,LOCATION_MZONE+LOCATION_HAND+LOCATION_DECK,0,1,1,nil,tp)
	local tc=g:GetFirst()
