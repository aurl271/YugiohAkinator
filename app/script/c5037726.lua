...
function c5037726.rfilter(c,tp)
	return c:IsAttribute(ATTRIBUTE_LIGHT)
		and Duel.GetMZoneCount(tp,c)>0 and (c:IsControler(tp) or c:IsFaceup())
end
function c5037726.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	e:SetLabel(1)
	if chk==0 then return Duel.CheckReleaseGroup(tp,c5037726.rfilter,1,nil,tp) end
	local g=Duel.SelectReleaseGroup(tp,c5037726.rfilter,1,1,nil,tp)
