...
function c6075801.spcfilter(c,rc,tp)
	local g=Group.FromCards(c,rc)
	return Duel.GetMZoneCount(tp,g)>0
end
function c6075801.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsReleasable() and Duel.CheckReleaseGroup(tp,c6075801.spcfilter,1,c,c,tp) end
	local rg=Duel.SelectReleaseGroup(tp,c6075801.spcfilter,1,1,c,c,tp)
	rg:AddCard(c)
...
function c6075801.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c6075801.spfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
