...
function c98280324.cfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xa9) and c:IsAbleToHandAsCost() and Duel.GetMZoneCount(tp,c)>0
end
function c98280324.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c98280324.cfilter,tp,LOCATION_MZONE,0,1,e:GetHandler(),tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectMatchingCard(tp,c98280324.cfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler(),tp)
