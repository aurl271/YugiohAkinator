...
function c28754338.cfilter(c,rc,tp)
	local g=Group.FromCards(c,rc)
	return c:IsRace(RACE_SEASERPENT) and Duel.GetMZoneCount(tp,g)>0
end
function c28754338.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsReleasable() and Duel.CheckReleaseGroup(tp,c28754338.cfilter,1,c,c,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	local rg=Duel.SelectReleaseGroup(tp,c28754338.cfilter,1,1,c,c,tp)
	rg:AddCard(c)
...
function c28754338.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c28754338.filter,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil,e,tp) end
