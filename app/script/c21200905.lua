...
function c21200905.cfilter(c,g,tp)
	return g:IsContains(c) and Duel.GetMZoneCount(tp,c)>0
end
function c21200905.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	local lg=e:GetHandler():GetLinkedGroup()
	if chk==0 then return Duel.CheckReleaseGroup(tp,c21200905.cfilter,1,nil,lg,tp) end
	local g=Duel.SelectReleaseGroup(tp,c21200905.cfilter,1,1,nil,lg,tp)
...
function c21200905.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c21200905.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp) end
